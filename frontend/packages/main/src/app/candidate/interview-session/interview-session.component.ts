...
export class InterviewSessionComponent implements OnInit, OnDestroy {
  ...
  loadSessionInfo(): void {
    ...
    this.candidateService.getSessionByToken(this.sessionToken).subscribe({
      next: (data: any) => {
        ...
        this.hasCodingChallenge = !!this.session.hasCodingChallenge;
    ...
    this.timerSubscription = interval(1000)
      ...
      .subscribe(() => {
        ...
        if (this.timeRemaining <= 300 && this.webcamInitialized) {
          if (Date.now() - this.lastSnapshotTime > 30000) {
            // Every 30 seconds
  ...
  captureSnapshot(eventType: string = "NORMAL"): void {
    if (
      !this.webcamInitialized ||
      !this.webcamVideo?.nativeElement ||
      !this.webcamCanvas?.nativeElement
    ) {
      return;
    }
  ...
  get mcqOptionsArray(): FormArray {
    return this.mcqForm.get('selectedOptions') as FormArray;
  }

  initMCQForm(options: any[]) {
    const controls = options.map(() => new FormControl(false));
    this.mcqForm.setControl('selectedOptions', new FormArray(controls, Validators.required));
  }

  getMCQOptionControl(optionId: string): FormControl {
    const index = this.questions[this.currentQuestionIndex].options.findIndex((o: any) => o.id === optionId);
    return this.mcqOptionsArray.at(index) as FormControl;
  }
  ...
  submitMCQAnswer(): void {
    ...
    const selectedOptions = this.questions[this.currentQuestionIndex].options
      .filter((option: any, idx: number) => this.mcqOptionsArray.at(idx).value)
  ...
  submitOpenEndedAnswer(): void {
    ...
    if (this.detectPotentialCopyPaste(textAnswer)) {
      ...
      this.proctorService
        .recordCopyPaste(this.sessionToken, textAnswer.substring(0, 100))
        .subscribe();
    }
    ...
    this.candidateService
      .submitOpenEndedAnswer(this.sessionToken, currentQuestion.id, textAnswer)
      .subscribe({
        next: (response: any) => {
          this.hasAnswered[this.currentQuestionIndex] = true;
          this.submitting = false;
  ...
  requestFullScreen(): void {
    if (this.fullscreenContainer && !this.isFullScreen) {
      this.lockdownService.enforceFullScreen(
  ...
  detectPotentialCopyPaste(content: string): boolean {
    ...
    const hasCodePatterns =
      /function|class|public|private|import|SELECT|FROM|WHERE|<div|<span/i.test(
        content
      );
  ...
  handleVisibilityChange = (): void => {
    if (document.hidden) {
      ...
      if (this.tabSwitchWarningCount > this.maxTabSwitchWarnings) {
        this.error =
          "Too many tab switches detected. Your activity has been flagged for review.";
      }
    } else {
    ...
    if (!this.isFullScreen && this.lockdownEnforced) {
      ...
      if (this.sessionToken) {
        this.lockdownService
          .recordFullscreenExit(this.sessionToken)
          .subscribe();
      }
      ...
      if (this.fullScreenWarningCount <= this.maxFullScreenWarnings) {
        ...
        this.error =
          "Too many fullscreen exits detected. Your activity has been flagged for review.";
      }
    }
  };
  ...
  allQuestionsAnswered(): boolean {
    return (
      this.hasAnswered.every((answered) => answered) &&
      this.hasAnswered.length > 0
    );
  }
