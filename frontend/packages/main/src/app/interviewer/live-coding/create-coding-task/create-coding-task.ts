...
import { NeoButtonComponent, NeoCardComponent, NeoInputComponent, NeoSelectComponent, NeoTextareaComponent, NeoCodeEditorComponent } from '@autojudge/ui/dist';
import { CodingService } from '../../../shared/services/coding.service';
import { CodingTask, CodingTaskType, ProgrammingLanguage, TestCase } from '../../../shared/models/coding.model';
...
export class CreateCodingTaskComponent implements OnInit {
  ...
  taskTypes = Object.values(CodingTaskType);
  ...
  private initForm(): void {
    this.taskForm = this.fb.group({
      ...
      timeLimit: [30, [Validators.required, Validators.min(5), Validators.max(120)]],
  ...
  createTask(): void {
    ...
    const testCases: TestCase[] = formValue.testCases.map((testCase: any, index: number) => ({
      ...testCase,
      order: index
    }));
    ...
    this.codingService.createCodingTask(codingTask).subscribe({
      ...
      error: (err) => {
        this.isSubmitting = false;
        this.errorMessage = err.error?.message || 'Failed to create coding task. Please try again.';
